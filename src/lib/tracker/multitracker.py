...
class JDETracker(object):
    ...
    def update_f3(self, im_blob, img0):
        ...
        id_features=np.array(id_features).permute(1,0,2).contiguous()
        hms=np.array(hms).permute(1,0,2).contiguous().squeeze()
        ...
        for x in range(9):
            ...
            if np.sum(hms[index_x,np.arange(len(index_x))]>=0.7)!=len(index_x) and x != 0:
